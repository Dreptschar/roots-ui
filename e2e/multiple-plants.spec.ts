...
test('prevents duplicate same-day watering logs for two scheduled plants', async ({ page }) => {
  const plants = [
  ...
  for (const plant of plants) {
    ...
    const duplicateDialog = page.getByRole('dialog', { name: 'Log action' });
    ...
    await expect(duplicateDialog.getByRole('status')).toContainText('already logged for today');
    ...
    await expect(recentActions.locator('.scheduleCard')).toHaveText(/Watering/);
